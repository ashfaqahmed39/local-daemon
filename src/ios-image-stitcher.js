...
const getAnchorSeam = (previousAnchors, nextAnchors, scrollRect, viewportRect) => {
  const groupUniqueAnchors = (anchors) => {
    ...
    for (const anchor of anchors || []) {
      grouped.set(anchor.key, grouped.has(anchor.key) ? null : anchor)
    }
    return grouped
  }
  ...
  for (const [key, previousAnchor] of previous) {
    ...
    const previousCut = viewportRect.top
      + (previousAnchor.y - scrollRect.sourceRect.y) * scrollRect.scaleY
    const nextCut = viewportRect.top
      + (nextAnchor.y - scrollRect.sourceRect.y) * scrollRect.scaleY
    ...
    candidates.push({
      previousCut: Math.round(previousCut),
      nextCut: Math.round(nextCut),
    })
  }
...
export const stitchIosFrames = async (frames, scrollRect) => {
  ...
  for (let index = 1; index < frames.length; index += 1) {
    const anchorSeam = getAnchorSeam(
      frames[index - 1].anchors,
      frames[index].anchors,
      scrollRect,
      rect,
    )
    if (anchorSeam) {
      seams.push(anchorSeam)
      continue
    }
